	...
	if(!ithreads && VERBOSELOOP) printf("DEVICE: \u2460 BLOCK x %d y %d z %d "
			"distrib number %d threads + block position in grid %d block position in grid %d\n",
			blockIdx.x, blockIdx.y, blockIdx.z, distrib_number, itb, itc);
