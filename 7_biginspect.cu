...
//	float MaxDispos = 0.0f, SumDispos = 0.0f;
	...
	verbosefile << "HOST: \u24F3 *************************BigLoop terminated ***************************"
			<< endl;
	verbosefile << "HOST: \u24F3 ***********************************************************************"
			<< endl << endl;
	...
	for (int arg = 0; arg < ASCRATCH * tile.NbTileXY; arg++)
		SumScratchpad += *(val_scratchpad + arg);
	for (int arg = 0; arg < ASCRATCH * tile.NbTileXY; arg++)
		MaxScratchpad = max(MaxScratchpad, *(val_scratchpad + arg));
	...
	verbosefile << "SCRATCHPAD \u24EC Path to Scratchpad validation  ....." << ScratchpadVal2Imagefile
			<< endl;
	...
	for (int i = 0; i < TA.MP * ADistrib; i++) {
		...
		if (tempa < XDistrib * YDistrib_extended)
			j_distribpos[tempa + tempb * XDistrib * YDistrib_extended] = 255.0 * test2_distrib[i]
					/ Maxdistrib;			// Validation image value
	}
